    ...
    def __init__(self, project: Project):
        ...
        for application in get_test_applications():
            ...
            self.procedures.update(
                {
                    Path(procedure.pathname).stem: procedure
...
class TestTeeResults(TestResults):
    def __init__(self, *args, **kwargs):
        ...
        for file in self.application.result_files:
            ...
            for record in file.result_records:
                passed = len(
                    [result for result in record.results if result.status == 1]
                )
                failed = len(
                    [result for result in record.results if result.status == 0]
                )
        ...
        self.metadatas[TEEMETRICS.PASSED_PROCEDURES] = len(
            [file for file, status in file_results.items() if status[1] == 0]
        )
        self.metadatas[TEEMETRICS.FAILED_PROCEDURES] = len(
            [file for file, status in file_results.items() if status[1] != 0]
        )
        self.metadatas[TEEMETRICS.PASSED_RECORDS] = len(
            [record for record, status in record_results.items() if status[1] == 0]
        )
        self.metadatas[TEEMETRICS.FAILED_RECORDS] = len(
            [record for record, status in record_results.items() if status[1] != 0]
        )
    ...
    def dump(self, path: Path):
        ...
        if len(self.application.result_files) == 0:
            ...
            record_count = reduce(
                lambda x, y: x + y,
                [len(procedure.records) for procedure in self.procedures.values()],
                0,
            )
            ...
            root.set(
                "tests",
                str(
                    self.metadatas[TEEMETRICS.FAILED_RECORDS]
                    + self.metadatas[TEEMETRICS.PASSED_RECORDS]
                ),
            )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        "--verbose",
        action="store_true",
        help="Display the results on the standard output",
    )
    ...
    if len(get_projects()) == 0:
        ...
        test_path = get_projects()[0].get_scalar_tool_prop_def(
